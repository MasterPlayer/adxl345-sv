...
int axi_adxl_set_wakeup(axi_adxl* ptr, enum wakeup_enum wakeup) {
	...
	switch (adxl_dev_get_power_ctl(ptr->dev) & POWER_CTL_WAKEUP_MASK) {
		case WAKEUP_8HZ: printf("8 Hz "); break;
		case WAKEUP_4HZ: printf("4 Hz "); break;
		case WAKEUP_2HZ: printf("2 Hz "); break;
		case WAKEUP_1HZ: printf("1 Hz "); break;
		default: printf("<undefined> "); return ADXL_UNCORRECT_VALUE; break;
	}
...
int axi_adxl_get_wakeup(axi_adxl* ptr, enum wakeup_enum *wakeup) {
	...
	*wakeup = (adxl_dev_get_power_ctl(ptr->dev) & POWER_CTL_WAKEUP_MASK);
	...
	switch (*wakeup) {
		...
		case WAKEUP_2HZ: printf("2 Hz "); break;
		case WAKEUP_1HZ: printf("1 Hz "); break;
		default: printf("<undefined> "); status = ADXL_UNCORRECT_VALUE; break;
	}
	printf("\r\n");
	return status;
}
...
int axi_adxl_set_range(axi_adxl* ptr, enum range_enum range) {
	...
	switch (range) {
		case RANGE_2G: printf("[-2G..2G] <10 bit> Sensitivity %1.4f g ", SCALE_FACTOR_2G); break;
		case RANGE_4G: printf("[-4G..4G] <10 bit> Sensitivity %1.4f g ", SCALE_FACTOR_4G); break;
		case RANGE_8G: printf("[-8G..8G] <10 bit> Sensitivity %1.4f g ", SCALE_FACTOR_8G); break;
		case RANGE_16G: printf("[-16G..16G] <10 bit> Sensitivity %1.4f g ", SCALE_FACTOR_16G); break;
		case RANGE_2G_FULL: printf("[-2G..2G] <10 bit> Sensitivity %1.4f g ", SCALE_FACTOR_FULL_RES); break;
		case RANGE_4G_FULL: printf("[-4G..4G] <11 bit> Sensitivity %1.4f g ", SCALE_FACTOR_FULL_RES); break;
		case RANGE_8G_FULL: printf("[-8G..8G] <12 bit> Sensitivity %1.4f g ", SCALE_FACTOR_FULL_RES); break;
		case RANGE_16G_FULL: printf("[-16G..16G] <13 bit> Sensitivity %1.4f g ", SCALE_FACTOR_FULL_RES); break;
		default: printf("\r\n"); return ADXL_UNCORRECT_VALUE; break;
	}
...
int axi_adxl_set_fifo_mode(axi_adxl* ptr, enum fifo_mode_enum fifo_mode) {
	...
	switch (adxl_dev_get_fifo_ctl(ptr->dev) & FIFO_CTL_FIFO_MODE) {
		case FIFO_MODE_BYPASS: printf("BYPASS "); break;
		case FIFO_MODE_FIFO: printf("FIFO "); break;
		case FIFO_MODE_STREAM: printf("STREAM "); break;
		case FIFO_MODE_TRIGGER: printf("TRIGGER "); break;
		default: printf("<undefined> "); return ADXL_UNCORRECT_VALUE; break;
	}
