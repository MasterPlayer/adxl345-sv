...
Несмотря на то, что прослойка обладает своим функционалом, API должно позволять взаимодействовать с устройством напрямую, без прослойки. В таком случае, софтовая часть должна предусматривать взаимодействие с регистрами устройства напрямую, используя стандартный I2C контроллер.
...
Необходимо провести исследование, насколько быстрее стал отрабатывать обработчик прерывания в связке с компонентом по сравнению с классическим вариантом работы. 

### Время работы обработчика прерывания процессора

Для организации тестирования времени выполнения обработки прерывания процессором используется компонент таймера-усреднителя + API Для него. 
Внешний репозиторий: [axi_timer_averager](https://github.com/MasterPlayer/axi-timer-averager)


Так как это напрямую влияет на производительность, нужно произвести измерение для того чтобы знать насколько быстрее отрабатывает обработчик прерывания в связке с компонентом против обработки напрямую. Измерение будет касаться только времени работы обработчика прерывания. 

#### Классический вариант работы при прямом соединении процессора с контроллером I2C и системы прерывания к компоненту

Схему придется модифицировать и включить в работу таймер, который будет измерять работу функции обработчика прерывания. Все отладочные выводы данных, printf необходимо убрать, так как вывод выполняется через терминал UART, скорость которого достаточно низкая, что может влиять на производительность. Набор функций для работы необходимо взять тот же, что используется при работе с компонентом. 

![изображение](https://user-images.githubusercontent.com/45385195/193815935-c5312354-2dea-46ab-80c6-59f7f8e80c23.png)

Взаимодействие всех элементов представлено на рисунке 

![изображение](https://user-images.githubusercontent.com/45385195/193816506-7ae8b975-fa03-4330-8162-0a10b253bd9c.png)

#### Вариант работы при использовании компонента

Здесь таймер будет стартовать по сигналу прерывания от компонента, а не от устройства, так как нам важно проверить за сколько времени выполняется обработчик прерывания процессора. Набор функций аналогичный классическому варианту работу. Схема тестирования представлена на рисунке 

![изображение](https://user-images.githubusercontent.com/45385195/193818242-d5e89b3c-2fb0-4f8f-8087-05f66bb5d95a.png)

Взаимодействие элементов представлено на рисунке

![изображение](https://user-images.githubusercontent.com/45385195/193818317-2754995e-1409-478d-aa7c-98ed6328b783.png)


# Лог изменений
Старт лога начинается с версии 2.1
